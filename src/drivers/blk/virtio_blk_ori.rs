...
pub fn read(sector: usize, count: usize, buf: usize) {
    debug!("read sector {} count {} buf 0x{:x}", sector, count, buf);
    io(sector, count, buf, Read);
}

pub fn write(sector: usize, count: usize, buf: usize) {
    debug!("write sector {} count {} buf 0x{:x}", sector, count, buf);
...
fn blk_irqhandler(status: &Box<u8>) {
    loop {
        ...
        if **status == constants::VIRTIO_BLK_S_OK {
            return;
        } else if **status == constants::VIRTIO_BLK_S_IOERR {
            panic!("VIRTIO_BLK_S_IOERR");
        } else if **status == constants::VIRTIO_BLK_S_UNSUPP {
