...
use crate::drivers::pci::PciAdapter;
...
impl ComCfg {
    ...
    pub fn reset_dev(&mut self) {
        debug!("init dev reset_dev at {:#p}", &self.com_cfg.device_status);
    ...
    pub fn dev_features(&mut self) -> u64 {
        ...
        // debug!(
        //     "dev features select addr {:#p} feature addr {:#p}",
        //     &self.com_cfg.device_feature_select, &self.com_cfg.device_feature
        // );
        ...
        // debug!(
        //     "dev features com_cfg device_feature_select {:#x}",
        //     self.com_cfg.device_feature_select
        // );
...
fn read_cap_raw(adapter: &PciAdapter, register: u32) -> PciCapRaw {
    ...
    trace!("Converting read word from PCI device config space into native endian bytes.");
