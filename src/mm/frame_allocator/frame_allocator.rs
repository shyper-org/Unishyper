...
pub fn init() -> Result<(), &'static str> {
    ...
    let free_frames = FrameRange::from_phys_addr(
        PAddr::new(paged_range().start).unwrap(),
        paged_range().end - paged_range().start,
    );
    free_regions[0] = Some(PhysicalMemoryRegion::new(
        free_frames,
        MemoryRegionType::Free,
    ));
    let reserved_regions: [Option<PhysicalMemoryRegion>; 32] = Default::default();

    let free_physical_memory_areas = free_regions.iter().flatten();
    let reserved_physical_memory_areas = reserved_regions.iter().flatten();
...
impl fmt::Debug for Chunk {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{:?} Chunk [{:?}]", self.typ, self.frames)
    }
}
