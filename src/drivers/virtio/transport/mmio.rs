...
impl MmioRegisterLayout {
	...
	pub fn set_ring_addr(&mut self, sel: u32, addr: usize) {
		unsafe {
			write_volatile(&mut self.queue_sel, sel);
			write_volatile(&mut self.queue_desc_low, addr as u32);
			write_volatile(&mut self.queue_desc_high, (addr >> 32) as u32);
		}
	}

	pub fn set_drv_ctrl_addr(&mut self, sel: u32, addr: usize) {
		unsafe {
			write_volatile(&mut self.queue_sel, sel);
			write_volatile(&mut self.queue_driver_low, addr as u32);
			write_volatile(&mut self.queue_driver_high, (addr >> 32) as u32);
		}
	}

	pub fn set_dev_ctrl_addr(&mut self, sel: u32, addr: usize) {
		unsafe {
			write_volatile(&mut self.queue_sel, sel);
			write_volatile(&mut self.queue_device_low, addr as u32);
			write_volatile(&mut self.queue_device_high, (addr >> 32) as u32);
		}
	}
